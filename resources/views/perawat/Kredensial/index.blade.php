...
@section('main')
    ...
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Request Kredensial</h1>
        </div>
        <!-- Content Row -->
        <div class="row">
            <div class="col-md-12">
...
<script>
    $(document).ready( function () {
        var table = $('#table').DataTable( {
            ...
            ajax: "{{ url('/perawat/Kredensial-masuk/data') }}",
                "columns": [
                    {
                        "data": "id_kredensial",
                        class: "text-center",
    ...
    $(document).on('click', '#add', function() {
        ...
        $('#form').attr('action', '{{ url('perawat/Kredensial-masuk/create') }}');     
    });

    $(document).on('click', '#check', function() {
        var id = $(this).data('id');
        
        window.location.href = "{{ url('perawat/Kredensial-masuk/detail/') }}/"+id;
    });
    ...
    $(document).on('click', '#delete', function() {
            ...
            if (confirm("Anda Yakin ingin menghapus data?")){
                $.ajax({
                    url : "{{ url('perawat/Kredensial-masuk/delete') }}/"+id,
                    success :function () {
                        location.reload();
                    }
                })
